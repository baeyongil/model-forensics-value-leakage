...
def test_local_ineligibility_skips_paid_replacement_classification(
    ...
    # Force the token gate to fail only in the second parameterized case.
    if status == "skipped_local_token_ineligible":
        rows = []
        for row in intermediates:
            payload = row.as_dict(include_hash=False)
            if row.arm == "resample":
                payload["replacement_token_ids"] = [1, 2, 3, 4, 5]
                payload["replacement_token_ids_hash"] = stable_hash([1, 2, 3, 4, 5]).replace(
                    "sha256:", ""
                )
            payload["record_hash"] = stable_hash(payload)
            rows.append(payload)
        # The deliberately hand-built hash above is rejected; use the module's
        # raw-token hash through a temporary valid record instead.
        from model_forensics.resample_runner import _token_hash

        for payload in rows:
            if payload["arm"] == "resample":
                payload["replacement_token_ids_hash"] = _token_hash(
                    payload["replacement_token_ids"]
                )
                body = {key: value for key, value in payload.items() if key != "record_hash"}
                payload["record_hash"] = stable_hash(body)
        intermediates = tuple(ResamplingGenerationRecord.from_dict(row) for row in rows)
